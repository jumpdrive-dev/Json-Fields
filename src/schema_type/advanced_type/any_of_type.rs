...
impl<const U: usize> From<[SchemaType; U]> for AnyOfType {
    fn from(value: [SchemaType; U]) -> Self {
        AnyOfType {
            variants: value.into_iter().collect(),
        }
    }
}
...
mod tests {
    ...
    fn single_any_of_variant_is_checked_correctly() {
        let enum_type = AnyOfType::from([SchemaType::Basic(BasicType::String)]);

        assert_eq!(enum_type.validate(&json!("")), Ok(()));
        assert_eq!(
            enum_type.validate(&json!(10)),
            Err(AnyOfTypeError(vec![SchemaType::Basic(BasicType::String),]))
        );
        assert_eq!(
            enum_type.validate(&json!(null)),
            Err(AnyOfTypeError(vec![SchemaType::Basic(BasicType::String),]))
        );
    }
    ...
    fn nested_any_of_are_checked_correctly() {
        let enum_type = AnyOfType::from([
            SchemaType::Advanced(AdvancedType::AnyOf(AnyOfType::from([SchemaType::Basic(
                BasicType::String,
            )]))),
            SchemaType::Advanced(AdvancedType::AnyOf(AnyOfType::from([SchemaType::Basic(
                BasicType::Number,
